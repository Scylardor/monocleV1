...
namespace moe
{
        ...
        DXGI_FORMAT requestedFormat, std::uint32_t viewportWidth, std::uint32_t viewportHeight)
    {
        ...
        MOE_ASSERT(SUCCEEDED(defaultOutput.As<IDXGIOutput1>(&output1)));
        ...
        MOE_ASSERT(SUCCEEDED(output1->GetDisplayModeList(requestedFormat, flags, &numModes, nullptr)));
        std::vector<DXGI_MODE_DESC> modeDescs(numModes);
        MOE_ASSERT(SUCCEEDED(output1->GetDisplayModeList(requestedFormat, flags, &numModes, modeDescs.data()) ));
